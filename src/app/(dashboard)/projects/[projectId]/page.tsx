...
function ProjectPage({ params }: ProjectPageProps) {
  ...
  const handleDocumentUpload = async (files: File[]) => {
    ...
    const newDocuments = await Promise.allSettled(uploadPromises);
    // update local state
  ...
  const handleUrlAdd = async (url: string) => {
    ...
    try{
      const token = await getToken();
      const result = await apiClient.post(`/api/projects/${projectId}/files/url`, {url}, token);

      const newData = result.data;
      // update local state
      setData((prev) => ({
        ...prev,
        documents: [newData, ...prev.documents],
      }))
